...
int main()
{
	...
	setbuf(log, NULL);
		...
		//设置超时
		const int snd_timeout = 1000;  //超时时长为1000ms
		...
		if (operation_code == 1)  //选择下载服务
		{
			...
			if (send_result == packet_length) {
				...
				sprintf(file_path,"TFTP_download_files/%s", download_filename);
				FILE* download_file = fopen(file_path, "wb");  //在本地开一个文件用于写入收到的数据
